...
class GATv2Conv(torch_geometric.nn.MessagePassing):
    _alpha: OptTensor
    def __init__(self, 
                 in_channels: int,
                 out_channels: int, 
    ...
    def forward(self,data, return_attention_weights: bool = None):
        r"""
        Args:
        ...
        # x: Union[Tensor, PairTensor], edge_index: Adj,
        x = data.x
    ...
    def message(self, x_j: Tensor, x_i: Tensor,
        ...
        if x_i.size(0) > 0:
            # 公式6
            self.embedding_l1 = x.norm(p=1) / x_i.size(0)
        else:
